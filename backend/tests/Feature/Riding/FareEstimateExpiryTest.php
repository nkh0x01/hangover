...
it('rejects ride creation when the fare estimate has expired', function (): void {
    ...
    expect(fn () => app(CreateRideRequestAction::class)->execute($customer, $dto))
        ->toThrow(RideNotOfferableException::class);
});

it('rejects ride creation when the fare estimate ulid is unknown', function (): void {
    $customer = User::factory()->create();

    $dto = new RideRequestData(
        fareEstimateUlid: '00000000000000000000000000',
        pickup: new Point(41.7151, 44.8271),
        pickupAddress: 'Pickup',
        dropoff: new Point(41.7321, 44.8271),
        dropoffAddress: 'Dropoff',
