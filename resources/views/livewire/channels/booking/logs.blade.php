    ...
    <div class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
        <table class="min-w-full divide-y divide-slate-200 text-sm">
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse ($logs as $log)
                    <tr>
                        <td class="px-4 py-2 text-xs text-slate-500">{{ $log->started_at?->format('Y-m-d H:i:s') }}</td>
                        <td class="px-4 py-2">
                        ...
                        <td class="px-4 py-2 text-xs text-slate-500">
                            ...
                            @elseif (is_array($log->payload_summary) && $log->payload_summary)
                                <code class="rounded bg-slate-50 px-1.5 py-0.5">{{ \Illuminate\Support\Str::limit(json_encode($log->payload_summary), 80) }}</code>
                            @else
        ...
        @if ($logs->hasPages())
            <div class="border-t border-slate-100 px-4 py-2">{{ $logs->links() }}</div>
        @endif
    </div>
</div>
