    ...
    if (!$signedName) {
        ...
        if (@file_put_contents($sigSavePath, $data)) {
            ...
            if (!$error) {
                ...
                $cardLink = SITE_URL . '/g_group.php?t=' . $group['group_token'];
                try {
                    queueSmsNow('warranty_signed', $group['phone'],
                        'გაჯეტი: მადლობა შეძენისთვის! თქვენი საგარანტიო ბარათები: ' . $cardLink,
                    ...
                    try {
                        ...
                        sendEmail($group['customer_email'], 'საგარანტიო ბარათები აქტიურია',
                            "<div style='font-family:Arial;max-width:520px;margin:0 auto'>
                             ...
                             <ul>{$li}</ul>
                             <p><a href='{$cardLink}' style='background:#111;color:#fff;padding:10px 20px;text-decoration:none;border-radius:6px'>ბარათების ნახვა</a></p></div>");
                    } catch (Throwable $e) { error_log('group email: ' . $e->getMessage()); }
                }

                if (ob_get_level()) { ob_end_clean(); }
                header('Location: ' . $cardLink);
                exit;
            }
        } else {
            $error = 'ხელმოწერის შენახვა ვერ მოხერხდა';
