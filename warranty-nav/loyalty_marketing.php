...
<style>
  :root{--ink:#0F172A;--muted:#64748B;--line:#E7EBF1;--bg:#F6F8FC;--primary:#4F46E5;--green:#059669;--red:#DC2626;--amber:#D97706}
  *{box-sizing:border-box;margin:0;padding:0}
  body{font-family:system-ui,"Noto Sans Georgian",sans-serif;background:var(--bg);color:var(--ink);font-size:14px}
  .top{background:#fff;border-bottom:1px solid var(--line);padding:14px 22px;display:flex;justify-content:space-between;align-items:center}
  .top a{color:var(--muted);text-decoration:none;font-size:13px}
  .wrap{max-width:1150px;margin:0 auto;padding:22px}
  h1{font-size:22px;margin-bottom:4px}.sub{color:var(--muted);margin-bottom:20px}
  .cards{display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin-bottom:18px}
  .card{background:#fff;border:1px solid var(--line);border-radius:12px;padding:16px}
  .card .v{font-size:26px;font-weight:800}.card .l{color:var(--muted);font-size:12px;margin-top:2px}
  .chips{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:18px}
  .chip{display:inline-block;padding:9px 15px;border-radius:999px;border:1px solid var(--line);background:#fff;
        text-decoration:none;color:var(--ink);font-size:13px;font-weight:600}
  .chip small{color:var(--muted);font-weight:700}
  .chip.on{background:var(--primary);border-color:var(--primary);color:#fff}
  .chip.on small{color:#C7D2FE}
  .bar{display:flex;gap:10px;align-items:center;margin-bottom:16px;flex-wrap:wrap}
  .bar input{padding:9px 12px;border:1px solid var(--line);border-radius:9px;font-size:13px;font-family:inherit;min-width:220px}
  .btn{background:var(--primary);color:#fff;border:0;border-radius:9px;padding:9px 16px;font-weight:600;cursor:pointer;font-size:13px;text-decoration:none;display:inline-block}
  ...
  .panel h2{font-size:15px;padding:13px 16px;border-bottom:1px solid var(--line);background:#FAFBFE}
  table{width:100%;border-collapse:collapse}
  th,td{padding:10px 14px;text-align:left;border-bottom:1px solid #EEF1F6;font-size:13px}
  th{background:#FAFBFE;color:var(--muted);font-size:11px;text-transform:uppercase;letter-spacing:.03em}
  .y{color:var(--green);font-weight:700}.n{color:#CBD5E1}
  .warn{background:#FFFBEB;border:1px solid #FDE68A;color:#92400E;padding:12px 14px;border-radius:10px;margin-bottom:16px;font-size:13px}
  .err{background:#FEF2F2;color:#991B1B;padding:13px;border-radius:10px;margin-bottom:16px;font-family:monospace;font-size:12.5px}
  .statuses{color:var(--muted);font-size:12px;margin:-8px 0 16px}
  @media(max-width:820px){.cards{grid-template-columns:repeat(2,1fr)}}
</style></head><body>
<div class="top"><b>💳 ლოიალურობა და მარკეტინგი</b><a href="index.php">← მთავარი</a></div>
<div class="wrap">
