...
it('lets active users through', function (): void {
    ...
    $response = (new EnsureNotBlocked)->handle($request, fn () => new Illuminate\Http\Response('ok'));
...
it('blocks suspended users with a 403 envelope', function (): void {
    ...
    expect(fn () => (new EnsureNotBlocked)->handle($request, fn () => new Illuminate\Http\Response('ok')))
